...
import type { SummaryGenerationContext, SummaryMeta } from "./summary-review.ts";
import {
  loadCuratorServer,
  loadExtract,
  loadGeminiSearch,
...
function renderSearchErrorPlan(
  ...
  expanded: boolean,
  theme: { fg: (key: string, s: string) => string; bg: (key: string, s: string) => string },
) {
  if (expanded) {
    return new Text(
...
function handleSessionChange(ctx: ExtensionContext): void {
  abortPendingFetches();
  closeCurator();
  void loadGithubExtract()
    .then((m) => m.clearCloneCache())
    .catch(() => {});
  sessionActive = true;
        ...
        },
        {
          ...
          async onAddSearch(query, queryIndex, provider) {
            ...
            const requestedProvider =
              !normalizedProvider || normalizedProvider === "auto" ? pc.searchProvider : normalizedProvider;
            try {
              const {
                answer,
                results,
      ...
      async execute(callId, params, signal, onUpdate, ctx) {
        ...
        if (shouldCurate) {
          ...
          const rawSearchProvider =
            normalizeProviderInput(params.provider ?? loadConfig().provider ?? "auto") ?? "auto";
        ...
        if (isPartial) {
          if (details?.phase === "curator-fallback") {
            ...
            lines.push(
              theme.fg(
                "dim",
                timeout ? `  auto-submits after ${timeout}s idle; ${shortcut} reopens` : `  ${shortcut} reopens`,
              ),
            );
            return new Text(lines.join("\n"), 0, 0);
          }
        ...
        const handle = await (
          ...
          },
          {
            ...
            async onAddSearch(query, queryIndex, provider) {
              ...
              const normalizedProvider = normalizeProviderInput(provider);
              const requestedProvider =
                !normalizedProvider || normalizedProvider === "auto" ? currentSearchProvider : normalizedProvider;
              try {
                const {
                  answer,
                  results,
