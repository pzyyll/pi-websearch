function e(e){return JSON.stringify(e).replace(/</g,`\\u003c`).replace(/>/g,`\\u003e`).replace(/&/g,`\\u0026`).replace(/\u2028/g,`\\u2028`).replace(/\u2029/g,`\\u2029`)}function t(e,t,n){return[{value:`openai`,label:`OpenAI`,available:e.openai},{value:`exa`,label:`Exa`,available:e.exa},{value:`brave`,label:`Brave`,available:e.brave},{value:`parallel`,label:`Parallel`,available:e.parallel},{value:`tavily`,label:`Tavily`,available:e.tavily},{value:`perplexity`,label:`Perplexity`,available:e.perplexity},{value:`gemini`,label:`Gemini`,available:e.gemini}].filter(e=>e.available).map(e=>{let r=e.value===t,i=r&&n?`loading`:`idle`,a=[`provider-btn`,i,r?`is-default`:``].filter(Boolean).join(` `),o=i===`loading`?` disabled`:``;return`<button type="button" class="${a}" data-provider="${e.value}" data-state="${i}"${o}>${e.label}</button>`}).join(``)}function n(n,a,o,s,c,l,u,d){let f=t(s,c,n.length>0),p=e({queries:n,sessionToken:a,timeout:o,defaultProvider:c,searchProvider:l,summaryModels:u,defaultSummaryModel:d,availableProviders:s});return`<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
...
<input type="text" id="timer-input" value="${o}">
...
<div class="provider-buttons" id="provider-buttons">${f}</div>
</div>
</div>
...
`,i=`(function() {
  ...
  function sanitizeHref(url) {
    ...
    return new RegExp("^https?://", "i").test(value) ? value : "#";
  }
...
})();`;export{n as generateCuratorPage};