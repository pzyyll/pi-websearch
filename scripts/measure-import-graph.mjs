...
function findHeavyLocalMarkers(files) {
  ...
  for (const key of HEAVY_LOCAL) {
    markers[key] = names.filter((n) => n === `${key}.ts` || n.endsWith(`/${key}.ts`) || n.includes(`${key}.`));
  }
  return markers;
}
...
async function maybeTimeJiti(entry) {
  ...
  let jitiFactory;
  const candidates = [() => require("jiti"), () => require(createRequire(join(ROOT, "package.json")).resolve("jiti"))];
